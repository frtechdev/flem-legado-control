...
public class ActiveDirectoryAction extends DispatchAction {
    ...
    public ActionForward unspecified(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            if ((nomeFiltro == null || nomeFiltro.isEmpty()) && request.getSession().getAttribute("adAction_filtro_nome") != null) {
                nomeFiltro = request.getSession().getAttribute("adAction_filtro_nome").toString();
            }
            request.getSession().setAttribute("adAction_filtro_nome", nomeFiltro);
            request.setAttribute("lista", new ActiveDirectoryBO().obterStatusUsuariosNoAD(new UsuarioBO().obterAtivosAD(nomeFiltro)));
        } catch (Exception ex) {
            ...
            Logger.getLogger(ActiveDirectoryAction.class.getName()).log(Level.SEVERE, ex.getMessage(), ex);
        }
    ...
    public ActionForward salvar(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            new BloqueioAfastamentoADBO().inserir(bloqueio);
    ...
    public ActionForward bloquear(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            new ActiveDirectoryBO().bloquearAcessos(funcionario, true);
            IUsuario usuario = (IUsuario) request.getSession().getAttribute(Constante.USUARIO_LOGADO);
            Log appLog = new Log();
            appLog.setLogin(usuario.getUsername());
    ...
    public ActionForward desbloquear(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            Usuario funcionario = new UsuarioBO().obterPorPk(Integer.valueOf(id));
            new ActiveDirectoryBO().bloquearAcessos(funcionario, false);
            IUsuario usuario = (IUsuario) request.getSession().getAttribute(Constante.USUARIO_LOGADO);
            Log appLog = new Log();
            appLog.setLogin(usuario.getUsername());
    ...
    public ActionForward excluir(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            BloqueioAfastamentoAD bloqueio = (BloqueioAfastamentoAD) dyna.get("bloqueio");
            new BloqueioAfastamentoADBO().excluir(bloqueio);
            IUsuario usuario = (IUsuario) request.getSession().getAttribute(Constante.USUARIO_LOGADO);
            Log appLog = new Log();
            appLog.setLogin(usuario.getUsername());
